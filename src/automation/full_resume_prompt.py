...
def load_template() -> str:
    ...
    if _template is None:
        _template = config.FULL_RESUME_PROMPT_PATH.read_text(encoding="utf-8")
    return _template
...
def build_messages(job_description: str, detailed_resume_text: str) -> list[dict]:
    return [
        {
            "role": "system",
            "content": "You are an expert resume writer and ATS optimizer. You always respond with plain text only, exactly in the format requested, never JSON or markdown fences.",
        },
